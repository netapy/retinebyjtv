  ...
  html2canvas(c).then(function (canvas) {
    canvas.toBlob(function (blob) {
      navigator.clipboard
        ...
        .then(function () {
          Swal.fire("Graphique copié !", "ctrl + v pour le coller nimporte où.", "success");
        });
    });
  });
}
...
async function queryRtn(link, method, body = "") {
...
async function dlData(num) {
  ...
  return fetch(`https://retinereq.jetevois.fr:8000/sondage/edit/x/${num}/`, requestOptions)
    .then((res) => res.blob())
    .then((blob) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      return fetch("https://retinereq.jetevois.fr:8000/sondages/edit/" + num + "/", requestOptions)
        .then((response) => response.text())
        .then((result) => {
          Swal.fire("Sondage supprimé", "Votre sondage ainsi que les réponses ont été supprimés.", "success");
...
function shareBtn(e) {
  navigator.clipboard.writeText("retine.jetevois.fr/sondage#" + e.toString()).then(
    function () {
      Swal.fire("Lien du sondage copié !", "retine.jetevois.fr/sondage#" + e.toString(), "success");
    },
    function () {
      console.log("error");
    }
